...
// Response schema for the opencode-prime release API. This fork only resolves
// versions from its own GitHub repository — it never consults the upstream
// opencode channels (npm, brew, choco, scoop).
  ...
  Effect.gen(function* () {
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        // opencode-prime is only distributed through its own GitHub releases via
        // install.sh / install.ps1. It never ships on npm/brew/choco/scoop, so the
        // install method is always "curl" regardless of where the binary lives.
        return "curl" as Method
      }),
      latest: Effect.fn("Installation.latest")(function* (_installMethod?: Method) {
        // The latest version is always resolved from the opencode-prime repository.
        // This fork never compares against the upstream opencode channels, so a
        // prime install can never be told about an upstream-only release.
        const response = yield* httpOk.execute(
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        // opencode-prime is only distributed through its own GitHub releases, so
        // the only supported upgrade path is the curl installer. Package-manager
        // upgrades would install the upstream opencode package, which is never
