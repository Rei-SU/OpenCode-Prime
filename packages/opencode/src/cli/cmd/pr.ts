...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run opencode-prime",
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found opencode-prime session: ${sessionUrl}`)
    ...
    UI.println()
    UI.println("Starting opencode-prime...")
    UI.println()
    ...
    if (code !== 0) return yield* Effect.die(new Error(`opencode-prime exited with code ${code}`))
  }),
})
