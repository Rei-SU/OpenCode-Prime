...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, opencode-prime does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`opencode-prime models\` to list available models`,
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`opencode-prime auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
